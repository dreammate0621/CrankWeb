...
app.factory('$radioScope', function ($rootScope)
{
...
app.controller('radioCtrl', function ($rootScope, $scope, $http, $uibModal,$timeout)
{

    //$scope.current_view = 'artist'; // artist or analytics
    $scope.current_view = 'radio'; // artist or analytics
    ...
    $http.get('data/radio_events.json')
     .then(function (res)
     {
    ...
    */
    $scope.$watch('current_view', function (val)
    {
        ...
        {
            case ('radio'):
                $scope.current_view_template = 'partials/radio_map.html';
                $scope.initMap();
                break;
            case ('analytics'):
    ...
    $scope.selectStation = function selectStation(station)
    {
       ...
       // console.log(artist);
        ...
        {
            return (_.findIndex(o.radio_staions, function (r)
                      {
                          return (r == $scope.current_station.call_code);
                      }) >= 0);
        });


        //Set filter
        $scope.showsFilter.radio_staions = $scope.current_station.call_code;
    ...
    $scope.selectEvent = function (event)
    {
        ...
        $scope.showsFilter.radio_staions = "";
        $scope.resetEventDates();
        $scope.addEventDates(filteredEvents);
        $scope.showTourDateScroller = true;
    ...
    * */
    ...
    $http.get('data/radio_stations.json')
    .success(function (res)
    {
        $scope.stations = res;
